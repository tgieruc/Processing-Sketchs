...
class Node{
  ...
  Node(float x_in, float y_in, float z_in){
    x = x_in;
    y = y_in;
    z = z_in;
  }
...
void setup() {
  ...
  for (int i = 0; i < nbr_nodes; ++i) {
     nodes.add( new Node(random(-maxsize_x,maxsize_x),random(-maxsize_y,maxsize_y),random(-maxsize_z,maxsize_z)));
  }
  for (Node temp : nodes) {
   temp.set_closest();
  }
...
void draw(){
  ...
  rotateY(turn);
  translate(width/2,height/2,-maxsize_x);
  turn +=0.02;
for (int num = 0; num < triangles.size(); ++num){
    ...
    if (draw){
        ...
        for (int i = 0 ; i < (nbr_line+ran) ; ++i) {
          ...
          (nodes.get(temp.item2).z - nodes.get(temp.item3).z)
          / (nbr_line+ran)*i+nodes.get(temp.item3).z),
          new Node((nodes.get(temp.item1).x - nodes.get(temp.item3).x)
          / (nbr_line+ran)*i+nodes.get(temp.item3).x,(nodes.get(temp.item1).y
          - nodes.get(temp.item3).y)/(nbr_line+ran)*i+nodes.get(temp.item3).y,
          (nodes.get(temp.item1).z - nodes.get(temp.item3).z)
          / (nbr_line+ran)*i+nodes.get(temp.item3).z));
        }
      }
  }
...
boolean shouldNotDrawTriangle(Triangle temp, Node n3, Node n4){
  ...
  Node n2 = new Node((nodes.get(temp.item2).x + nodes.get(temp.item3).x)/2,(nodes.get(temp.item2).y + nodes.get(temp.item3).y)/2,(nodes.get(temp.item2).z + nodes.get(temp.item3).z)/2);
...
float nodeDistance(Node n1, Node n2) {
  return sqrt((n2.x - n1.x)*(n2.x - n1.x) + (n1.y - n2.y)*(n1.y - n2.y)+(n1.z-n2.z)*(n1.z-n2.z));
}
